...
export const CartProvider = ({ children }) => {
  // provee de la info, se usa solo 1 vez haciendo wrap (en app) a los compon que quiero q accedan a la info, la pasa
  const [quantity, setQuantity] = useState(0);
  const [cart, setCart] = useState([]);

  const addToCart = (item, number) => {
    // para mi Purchase necesito: items[{id,,title,price}, date, total]
    const newCart = [...cart];
  ...
  return (
    <CartContext.Provider
      value={{ cart, addToCart, quantity, removeItem, clean }}
    >
      {children}
    </CartContext.Provider>
  );
